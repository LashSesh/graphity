...
/// Run the ISLS v3.1 template-free generation pipeline.
///
/// Replaces Tera-template emission with LLM-driven, compile-verified
/// generation.  Every file's prompt includes all types generated so far
/// (the growing TypeContext), eliminating hallucinated field names.
///
/// Pass `config.mock_oracle = true` to use mock generators (no API key needed).
pub fn forge_v31(
    requirements_path: &Path,
    output_dir: &Path,
    config: &DecomposerConfig,
    ...
    use isls_renderloop::{MockOracle, OpenAiOracle};

    let start = std::time::Instant::now();

    // 1. Parse TOML → HyperCube (same as forge_v2)
    let cube = isls_hypercube::parser::parse_toml_to_cube(requirements_path)?;
    let total_dims = cube.dimensions.len();
    let initial_dof = cube.dof();
    let total_couplings = cube.couplings.len();

    // 2. Detect domain → DomainTemplate
    let registry = DomainRegistry::new();
    let toml_content = std::fs::read_to_string(requirements_path)?;
    let domain = registry
        .detect(&toml_content)
    ...
    let app_name = cube.dimensions.iter()
        ...
        .unwrap_or_else(|| "warehouse-app".into());

    let domain_name = domain.name.clone();

    // 4. Build ForgePlan from domain
    let description = format!(
        "A {} application generated by ISLS v3.1.",
        domain_name
    );
    let plan = ForgePlan::from_domain(&app_name, &description, &domain);

    // 5. Create oracle
    let use_mock = config.mock_oracle || (config.api_key.is_none()
        && std::env::var("OPENAI_API_KEY").is_err());

    let oracle: Box<dyn isls_renderloop::Oracle> = if use_mock {
        Box::new(MockOracle)
    } else {
        match OpenAiOracle::new(config.api_key.clone(), Some(config.model.clone())) {
            Ok(o) => Box::new(o),
            Err(e) => {
                tracing::warn!("oracle init failed: {e}; falling back to mock");
                Box::new(MockOracle)
            }
        }
    };
    ...
    let generated_files = forge
        .generate()
        .map_err(|e| DecomposerError::Failed(format!("forge v3.1 failed: {e}")))?;
    ...
    Ok(ForgeV2Result {
        ...
        total_loc: generated_files.iter().map(|f| f.content.lines().count()).sum(),
        blueprint_hits: 0,
        template_hits: 0,
        trace: Vec::new(),
        time_secs,
