...
impl SimplexMesh {
    ...
    pub fn laplacian_matrix(&self) -> isls_topology::SparseLaplacian {
        let n = self.vertices.len();
        ...
        isls_topology::SparseLaplacian { n, degree, adjacency }
    }

    /// Spectral gap λ₁ via C16 decomposition.
    pub fn spectral_gap(&self, laplacian: &isls_topology::SparseLaplacian) -> f64 {
        if laplacian.n < 2 {
            return 0.0;
        }
        let k = laplacian.n.min(10);
        isls_topology::spectral_decompose(laplacian, k).spectral_gap
    }
    ...
    pub fn spectral_gradient(
        &self,
        vertex_id: usize,
        laplacian: &isls_topology::SparseLaplacian,
    ) -> Vec<f64> {
        ...
        let k = laplacian.n.min(10);
        let decomp = isls_topology::spectral_decompose(laplacian, k);
