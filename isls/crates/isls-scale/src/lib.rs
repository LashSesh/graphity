...
use isls_topology::{SpectralDecomposition, KuramotoState, kuramoto_order_parameter};
...
pub fn multi_scale_tick(
    micro_state: &isls_engine_types::MicroState,
...
fn build_scale_crystal(
    ...
    let mut crystal = isls_types::SemanticCrystal {
        crystal_id: [0u8; 32],
        region: region.clone(),
        constraint_program: Vec::new(),
        stability_score: stability,
        topology_signature: isls_types::TopologySignature::default(),
        betti_numbers: vec![universes.len() as u64, 0, 0],
        evidence_chain: Vec::new(),
        commit_proof: isls_types::CommitProof::default(),
    ...
    crystal.crystal_id = isls_types::content_address(&core);
...
pub mod isls_engine_types {
    use std::collections::BTreeMap;
    use isls_types::{FiveDState, VertexId};
    use isls_persist::PersistentGraph;
...
mod tests {
    ...
    use isls_topology::{compute_laplacian, spectral_decompose, init_kuramoto_state,
    ...
    fn at_sc14_downward_projection() {
        ...
        ks.phases = ks_phases;
        let (r, psi) = isls_topology::kuramoto_order_parameter(&ks.phases);
