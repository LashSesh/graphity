...
pub fn emit_v2(
    ...
    for entity in entities {
        ...
        // Service
        let service_ctx = build_service_context(entity, &table_name, &domain.business_rules);
        emit_simple(&tera, "v2_service_rs", &service_ctx,
            output_dir, &format!("backend/src/services/{}.rs", snake), "business_logic", &mut files)?;

        // API
        let api_ctx = build_api_context(entity, &table_name, &domain.business_rules);
...
fn build_query_context(entity: &EntityTemplate, table_name: &str, api: &isls_hypercube::domain::ApiFeatures) -> serde_json::Value {
    ...
    let insert_binds: Vec<String> = insert_fields.iter()
        .map(|f| {
            if f.nullable || f.rust_type.starts_with("Option<") {
    ...
    let update_binds: Vec<String> = update_fields.iter()
        .map(|f| format!("payload.{}.as_deref()", f.name))
        .collect();
...
fn build_service_context(entity: &EntityTemplate, table_name: &str, rules: &[BusinessRule]) -> serde_json::Value {
    let snake = to_snake_case(&entity.name);

    // Filters relevant to this entity
...
fn build_api_context(entity: &EntityTemplate, table_name: &str, rules: &[BusinessRule]) -> serde_json::Value {
    let snake = to_snake_case(&entity.name);
