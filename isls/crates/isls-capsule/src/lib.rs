...
use hkdf::Hkdf;
use sha2::Sha256;
use isls_types::Hash256;
use isls_manifest::ExecutionManifest;
...
fn compute_nonce(run_id: &Hash256, seal_counter: u64) -> [u8; 12] {
    ...
    let digest = isls_types::content_address_raw(&data);
...
pub fn verify_policy(capsule: &Capsule, manifest: &ExecutionManifest) -> std::result::Result<(), PolicyError> {
    ...
    // Program ID
    let manifest_program_digest = isls_types::content_address_raw(manifest.program_id.as_bytes());
...
mod tests {
    ...
    fn at_c6_wrong_manifest() {
        ...
        let rd2 = isls_types::RunDescriptor {
            config: Config::default(),
            operator_versions: BTreeMap::new(),
