...
use thiserror::Error;
use isls_types::{content_address, EvidenceEntry, GateSnapshot, Hash256, RunDescriptor};
...
/// Per-macro-step trace record (ISLS Extension Def T_k)
...
/// Execution manifest binding an entire run (ISLS Extension Def M)
#[derive(Clone, Debug, Serialize, Deserialize)]
...
/// Self-contained bundle sufficient for offline replay (ISLS Extension Def RP)
#[derive(Clone, Debug, Serialize, Deserialize)]
...
mod tests {
    use super::*;
    use isls_types::{Config, GateSnapshot};
    use isls_archive::Archive;
    use isls_registry::RegistrySet;
