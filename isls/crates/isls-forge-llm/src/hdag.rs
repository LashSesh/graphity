...
impl CodegenHdag {
    ...
    pub fn build(spec: &AppSpec) -> Self {
        ...
        // ── Layer 1: Foundation LLM nodes ────────────────────────────────────
        let errors_idx = add_node(
            "backend/src/errors.rs", NodeType::Llm, 1, None, true,
            "Define AppError enum with all error variants. Include actix_web::ResponseError impl.",
        );
        let config_idx = add_node(
            "backend/src/config.rs", NodeType::Llm, 1, None, true,
            "Define AppConfig struct loading from env: DATABASE_URL, JWT_SECRET, PORT.",
        );
        let pagination_idx = add_node(
            "backend/src/pagination.rs", NodeType::Llm, 1, None, true,
            "Define PaginationParams and PaginatedResponse<T>. Include SQL OFFSET helper.",
        );

        // ── Layer 2: Auth LLM nodes ───────────────────────────────────────────
        let user_model_idx = add_node(
            "backend/src/models/user.rs", NodeType::Llm, 2,
            Some("User".into()), true,
            "Define User struct, CreateUserPayload, UpdateUserPayload. Derive Serialize, Deserialize, sqlx::FromRow.",
        );
        let auth_idx = add_node(
            "backend/src/auth.rs", NodeType::Llm, 2, None, true,
            "JWT: Claims struct, AuthUser extractor, encode_jwt/decode_jwt. Use AppError::Unauthorized on failure.",
        );

        // errors → user_model
        edges.push(HdagEdge { from: errors_idx, to: user_model_idx, provides: provided::provides_apperror() });
        // errors → auth
        edges.push(HdagEdge { from: errors_idx, to: auth_idx, provides: provided::provides_apperror() });

        // ── Layer 3: Entity model LLM nodes ───────────────────────────────────
        let mut entity_model_indices: Vec<(String, usize)> = Vec::new();
        for entity in spec.entities.iter().filter(|e| e.name != "User") {
            let idx = add_node(
                &format!("backend/src/models/{}.rs", entity.snake_name),
                NodeType::Llm, 3, Some(entity.name.clone()), true,
                &format!(
                    "Define {} struct, Create{}Payload, Update{}Payload. Derive Debug, Clone, Serialize, Deserialize, sqlx::FromRow. Add validate() method.",
        ...
        // ── Layer 4: Database LLM nodes ───────────────────────────────────────
        let pool_idx = add_node(
            "backend/src/database/pool.rs", NodeType::Llm, 4, None, true,
            "pub async fn create_pool() -> Result<PgPool, AppError>. Load DATABASE_URL from env. No migrate!() macro.",
        );
        // errors → pool
        edges.push(HdagEdge { from: errors_idx, to: pool_idx, provides: provided::provides_apperror() });
...
mod tests {
    ...
    fn test_errors_node_provides_apperror() {
        let spec = test_spec();
        let hdag = CodegenHdag::build(&spec);
        let errors_idx = hdag.nodes.iter().find(|n| n.path.ends_with("errors.rs")).unwrap().index;
        // errors should have no incoming edges
        assert_eq!(hdag.predecessors(errors_idx).len(), 0, "errors.rs has no predecessors");
        // errors should provide symbols to at least auth
        let auth_idx = hdag.nodes.iter().find(|n| n.path.ends_with("auth.rs")).unwrap().index;
        let provided = hdag.provided_symbols(auth_idx);
        assert!(
            provided.iter().any(|s| s.import_path.contains("AppError")),
            "auth.rs must receive AppError symbol"
        );
    }
}
