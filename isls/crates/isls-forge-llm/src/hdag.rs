...
impl CodegenHdag {
    ...
    pub fn build(spec: &AppSpec) -> Self {
        ...
        // errors → user_model (user.rs is still LLM; it needs AppError as context)
...
mod tests {
    ...
    fn test_errors_node_provides_apperror() {
        ...
        // errors.rs still provides AppError to LLM nodes downstream (e.g. user model)
        let user_model_idx = hdag.nodes.iter().find(|n| n.path.ends_with("models/user.rs")).unwrap().index;
        let provided = hdag.provided_symbols(user_model_idx);
        assert!(
            provided.iter().any(|s| s.import_path.contains("AppError")),
            "user.rs must receive AppError symbol from errors.rs"
        );
    }
}
