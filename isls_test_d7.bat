...
if defined OPENAI_API_KEY (
    ...
    echo   !DIM!Verwende OPENAI_API_KEY aus Umgebung.!RST!
) else (
    echo   !DIM!Kein API-Key — Architect arbeitet im Manual-Modus.!RST!
)
echo.
...
echo   !DIM!Sende: "Build me a crypto trading journal with trades, portfolios, and performance metrics"!RST!
...
echo   !DIM!Sende: "Each trade has pair, side, entry_price, exit_price, quantity, timestamp, and notes"!RST!
...
:: Zeige Zusammenfassung statt voller JSON
...
echo   !DIM!Starte Forge fuer Session !SESSION_ID!...!RST!
...
if errorlevel 1 (
    ...
    echo   !DIM!Der Forge laeuft im Hintergrund. Pruefe nach ~30s:!RST!
    echo   !CYN!  curl !BASE!/api/session/!SESSION_ID!!RST!
    echo   !DIM!und suche nach "forge_result" im JSON.!RST!
    echo.
    echo   !DIM!Warte 15 Sekunden auf Ergebnis...!RST!
    ...
    if errorlevel 1 (
        ...
        echo   !DIM!Pruefe manuell mit: curl !BASE!/api/session/!SESSION_ID!!RST!
    ) else (
...
echo.
echo   !DIM!────────────────────────────────────────────────────!RST!
echo   !DIM!Session-ID: !SESSION_ID!!RST!
echo   !DIM!Server:     !BASE!!RST!
echo   !DIM!Studio:     !BASE!/studio!RST!
echo.
pause
endlocal
